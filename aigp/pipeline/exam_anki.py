#!/usr/bin/env python3
"""Build an Anki .apkg of the IAPP AIGP Practice Exam — one card per question.
...
<ul class="opts">
 ...
 {{#OptE}}<li class="{{OptECls}}"><span class="k">E</span>{{OptE}}</li>{{/OptE}}
</ul>
"""
QFMT = ('<div class="cue">AIGP · Domain {{Domain}} · Q{{Num}}</div>'
...
AFMT = ('<div class="cue">AIGP · Domain {{Domain}} · Q{{Num}}</div>'
        '{{#Scenario}}<div class="scenario"><b>Scenario {{ScenLabel}}.</b> {{Scenario}}</div>{{/Scenario}}'
...
MODEL = genanki.Model(
    MODEL_ID, "AIGP — Practice Exam Question",
    fields=[{"name":n} for n in
        ("Num","Domain","DomainName","Subdomain","ScenLabel","Scenario","Stem",
         "OptA","OptB","OptC","OptD","OptE","OptACls","OptBCls","OptCCls","OptDCls","OptECls",
...
def _ans_letters(q):
    return [x for x in re.split(r"[,\s]+", str(q["answer"])) if x]
def q_spoken(q):
    o=q["options"]
...
def concat_phase2(chunkmap, media_dir, failed):
    ...
    for (num,kind),paths in chunkmap.items():
        fname=f"aigpexam_q{num:02d}_{kind}.mp3"
...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if a.no_audio:
            ...
            work.rmdir()
        deck=genanki.Deck(DECK_ID, "AIGP · Practice Exam (100 questions + rationales)")
        media=[]
        for q in qs:
            letters=set(_ans_letters(q)); o=q["options"]
            cls={k:("correct" if k in letters else "") for k in "ABCDE"}
            ans_text=" · ".join(norm(o[k]) for k in ("A","B","C","D","E") if k in letters and k in o)
            clips=amap.get(q["num"],{})
            note=genanki.Note(model=MODEL, guid=genanki.guid_for(f"aigp-exam-q{q['num']}"),
                fields=[str(q["num"]), q["domain"] or "", q["domain_name"] or "", q["subdomain"] or "",
                        ...
                        norm(q["stem"]),
                        norm(o.get("A","")),norm(o.get("B","")),norm(o.get("C","")),norm(o.get("D","")),norm(o.get("E","")),
                        cls["A"],cls["B"],cls["C"],cls["D"],cls["E"],
                        str(q["answer"]), ans_text, norm(q["rationale"]),
                        snd(clips.get("q","")), snd(clips.get("a",""))],
                tags=[f"Domain-{q['domain']}", "AIGP-Practice-Exam"] +
