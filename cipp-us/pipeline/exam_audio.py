#!/usr/bin/env python3
"""Two-voice audio drill of the CIPP/US Practice Exam.
...
def a_script(q):
    L=q["answer"]; return f"The correct answer is {L}. {norm(q['options'][L])} {norm(q['rationale'])}"
...
def plan_parts(qs):
    ...
    for pi in range(n_parts):
        ...
        seq += spoken_chunks(f"intro{pi}",
            f"CIPP US practice exam. Audio drill, part {pi+1} of {n_parts}. "
...
def main():
    ...
    guide=["# CIPP/US Practice Exam — Audio Drill\n",
